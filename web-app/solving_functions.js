...
var options = [];
var before_hyposthesis = [];
var hypothesis_guess;
var hypothesis_coords;
...
help.update_box = function (box, quadrant) {
    const add_to_row = [0, 0, 0, 3, 3, 3, 6, 6, 6][quadrants.indexOf(quadrant)];
    const add_to_col = [0, 3, 6, 0, 3, 6, 0, 3, 6][quadrants.indexOf(quadrant)];
        ...
        for (let col = 0; col < 3; col++) {
            ...
            if (options[row + add_to_row][col + add_to_col] !== value) {
                if (!Array.isArray(value)) {
                    ...
                    options[row + add_to_row][col + add_to_col] = value;
                }
            }
        }
    }
    ...
    for (let quadrant = 0; quadrant < 9; quadrant++) {
        ...
        for (let number = 1; number < 10; number++) {
            ...
            for (let row = 0; row < 3; row++) {
                for (let col = 0; col < 3; col++) {
                    if (
                        eval("r" + rows[row] + ".includes(number)") ||
...
help.remove_from_row = function (section, number, row) {
    for (let col = 0; col < 9; col++) {
        if (
            (section === 0 && [3, 4, 5, 6, 7, 8].includes(col)) ||
...
help.remove_from_col = function (section, number, col) {
    for (let row = 0; row < 9; row++) {
        if (
            (section === 0 && [3, 4, 5, 6, 7, 8].includes(row)) ||
    ...
    for (let index = 0; index < 9; index++) {
        ...
        for (let number = 1; number < 10; number++) {
            ...
            if (count === 1) {
                ...
                if (type === "row")
                    help.update_instances(number, index, cell_index);
                if (type === "col")
                    help.update_instances(number, cell_index, index);
            }
        }
        help.update_cells(cells, type, index);
    }
...
help.init = function (strBoard) {
    ...
    options = [
        ...
        [
            parseInt(strBoard[9]),
            parseInt(strBoard[10]),
            parseInt(strBoard[11]),
            parseInt(strBoard[12]),
            parseInt(strBoard[13]),
            parseInt(strBoard[14]),
            parseInt(strBoard[15]),
            parseInt(strBoard[16]),
            parseInt(strBoard[17]),
        ],
        [
            parseInt(strBoard[18]),
            parseInt(strBoard[19]),
            parseInt(strBoard[20]),
            parseInt(strBoard[21]),
            parseInt(strBoard[22]),
            parseInt(strBoard[23]),
            parseInt(strBoard[24]),
            parseInt(strBoard[25]),
            parseInt(strBoard[26]),
        ],
        [
            parseInt(strBoard[27]),
            parseInt(strBoard[28]),
            parseInt(strBoard[29]),
            parseInt(strBoard[30]),
            parseInt(strBoard[31]),
            parseInt(strBoard[32]),
            parseInt(strBoard[33]),
            parseInt(strBoard[34]),
            parseInt(strBoard[35]),
        ],
        [
            parseInt(strBoard[36]),
            parseInt(strBoard[37]),
            parseInt(strBoard[38]),
            parseInt(strBoard[39]),
            parseInt(strBoard[40]),
            parseInt(strBoard[41]),
            parseInt(strBoard[42]),
            parseInt(strBoard[43]),
            parseInt(strBoard[44]),
        ],
        [
            parseInt(strBoard[45]),
            parseInt(strBoard[46]),
            parseInt(strBoard[47]),
            parseInt(strBoard[48]),
            parseInt(strBoard[49]),
            parseInt(strBoard[50]),
            parseInt(strBoard[51]),
            parseInt(strBoard[52]),
            parseInt(strBoard[53]),
        ],
        [
            parseInt(strBoard[54]),
            parseInt(strBoard[55]),
            parseInt(strBoard[56]),
            parseInt(strBoard[57]),
            parseInt(strBoard[58]),
            parseInt(strBoard[59]),
            parseInt(strBoard[60]),
            parseInt(strBoard[61]),
            parseInt(strBoard[62]),
        ],
        [
            parseInt(strBoard[63]),
            parseInt(strBoard[64]),
            parseInt(strBoard[65]),
            parseInt(strBoard[66]),
            parseInt(strBoard[67]),
            parseInt(strBoard[68]),
            parseInt(strBoard[69]),
            parseInt(strBoard[70]),
            parseInt(strBoard[71]),
        ],
        [
            parseInt(strBoard[72]),
            parseInt(strBoard[73]),
            parseInt(strBoard[74]),
            parseInt(strBoard[75]),
            parseInt(strBoard[76]),
            parseInt(strBoard[77]),
            parseInt(strBoard[78]),
            parseInt(strBoard[79]),
            parseInt(strBoard[80]),
        ],
    ];
...
help.hypothesis = function (input_options, hyp, row, col) {
    // Upload the new board
    options = input_options;

    // keep current board
    before_hyposthesis = [...input_options];
    hypothesis_guess = hyp;
    hypothesis_coords = [row, col];

    // gET value of board
    const cell = options[row][col];

    let response = {};

    if (Array.isArray(cell)) {
        if (cell.length === 2 && cell.includes(hyp)) {
            options[row][col] = hyp;
            response.options = options;
            response.changed = true;
        } else {
            response.changed = false;
            response.reason = "Incorrect cell or value not included in cell";
        }
    } else {
        response.changed = false;
        response.reason = "Incorrect cell or value not included in cell";
...
help.hypothesis_undo = function (input_options, hyp, row, col) {
    // get board before change
    options = input_options;

    // get other value
    let values = options[row][col];
    for (let index = 0; index < 2; index++) {
        const element = values[index];
        if (element === hyp) continue;
        else values = element;
    }

    options[row][col] = values;
    return options;
};
